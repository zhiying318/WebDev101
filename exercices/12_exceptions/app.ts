class Rectangle {
    constructor(private longueur: number, private largeur: number, private output: HTMLElement) {}
  
    calculerSurface(): number {
      if (this.longueur <= 0 || this.largeur <= 0) {
        throw new Error(
          `Les dimensions doivent être positives. Reçues: longueur=${this.longueur}, largeur=${this.largeur}`
        );
      }
  ...
  export function runRectangleTest(outputId: string): void {
